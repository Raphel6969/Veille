...
def print_packet(packet: dict[str, Any]) -> None:
    print("=" * 72)
    print(
        f"SCENARIO: {packet['scenario']}  PRESET: {packet['flag_preset']}  "
...
def _write_system_signals(packets: list[dict[str, Any]]) -> None:
    ...
    for p in packets:
        ...
        if c["repeated_groups"]:
            ...
            for g in c["repeated_groups"]:
                ...
                lines.append(
                    f"- `{g['tool']}` x{g['count']} — {tag} "
                    f"(idempotent={g.get('idempotent')}, matches={g['match_types']})"
                )
            lines.append(
                f"  summary: semantic_duplicates={c['summary_semantic_duplicates']}, "
    ...
    for p in packets:
        ...
        if t["plan_enabled"]:
            lines.append(
                f"- {p['scenario']}/{p['flag_preset']}: plan_tier={t['plan_tier']}, "
    ...
    for p in packets:
        ...
        if r["plan_enabled"] and r["routed_calls"]:
            for rc in r["routed_calls"]:
                lines.append(
                    f"- {p['scenario']}/{rc['step']}/{rc['agent']}: "
                    f"model={rc['model']} tier={rc['routing_tier']} "
                    f"reason={rc['routing_reason']}"
                )
    lines.append("\n## Q4 — Freshness defaults observed")
    lines.append(
        f"- cache default TTL = {CACHE_DEFAULT_TTL}s; memory default TTL = {MEMORY_DEFAULT_TTL}"
    )
